...
namespace WatchdogLib
{
    public class ApplicationHandler
    {
        ...
        public ApplicationHandler(string applicationName,
            string applicationPath,
            int nonResponsiveInterval,
            uint heartbeatInterval = 15,
            int minProcesses = 1,
            int maxProcesses = 1,
            bool keepExistingNoProcesses = false,
            bool useHeartbeat = false,
            bool grantKillRequest = true,
            uint startupMonitorDelay = 20,
            bool active = true)
        {
        ...
        public void HandleDuplicateProcesses()
        {
            //if (ProcessNo(ApplicationName) < MaxProcesses))
            {
                ...
                Logger.Error("multiple processes of application {0} are running, all but one will be killed ",
                    ApplicationName);
                ...
                foreach (var process in processes)
                {
                    ...
                    if (!process.Responding)
                    {
                        ...
                        var currResult = (processHandler != null)
                            ? processHandler.Kill()
        ...
        public bool HandleUnmonitoredProcesses()
        {
            try
            {
                ...
                foreach (var process in processes)
                {
                    if (ProcessHandlers.All(procHandle =>
                        procHandle.Process == null || procHandle.Process.Id != process.Id))
                    {
                        // This process o
        ...
        private void HandleExitedProcesses()
        {
            for (var index = 0; index < ProcessHandlers.Count; index++)
            {
                ...
                if (processHandler.HasExited)
                {
                    ...
                    var lessProcessesThanBefore =
                        (ProcessNo(processHandler.Name) < MaxProcesses) && KeepExistingNoProcesses;

                    if (notEnoughProcesses || lessProcessesThanBefore)
                    {
                        if (notEnoughProcesses)
                        {
                            Logger.Info("Process {0} has exited and no others are running, so start new",
                                processHandler.Name);
                        }
                        ...
                        {
                            Logger.Info(
                                "Process {0} has exited, and number of processed needs to maintained , so start new",
                                processHandler.Name);
                        }
        ...
        private void HandleNonResponsiveProcesses()
        {
            for (var index = 0; index < ProcessHandlers.Count; index++)
            {
                ...
                if (processHandler.IsStarting)
                {
                    ...
                    Debug.WriteLine("Process {0} is still in startup phase, no checking is being performed",
                        processHandler.Name);
                ...
                var noHeartbeat = _heartbeatServer.HeartbeatTimedOut(processHandler.Name, HeartbeatInterval) &&
                                  UseHeartbeat;
                ...
                var reason = notRespondingAfterInterval ? "not responding" :
                    noHeartbeat ? "not sending a heartbeat signal within hard limit" : "requesting to be killed";
                ...
                if (performKill)
                {
                    ...
                    {
                        Logger.Error("Process {0} was {1} and has been successfully killed ", processHandler.Name,
                            reason);
                    ...
                    else
                    {
                        ...
                        Logger.Error("Process {0} was {1} but could not be successfully killed ", processHandler.Name,
                            reason);

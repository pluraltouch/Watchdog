...
namespace WatchdogLib
{
    ...
    public class HeartbeatServer
    {
        ...
        public ISet<HeartbeatClient> Clients => _clients;
        ...
        public bool KillRequested(string processName)
        {
            ...
            if (performKill)
            {
                client.RequestKill = false; // Kill request only returns true once
            }
        ...
        private void OnClientMessage(NamedPipeConnection<string, string> connection, string message)
        {
            ...
            uint command;
            if (!uint.TryParse(args[0], out command))
            {
                return;
            }
            ...
            switch (command)
            {
                ...
                case (int) Commands.RequestKill:
                {
                    ...
                    if (args.Length == 3)
                    {
                        ...
                        Logger.Warn("Received kill after {0} seconds request by Process {1}", delay,
                            client.ProcessName);
        ...
        {
            _server.PushMessage(((int) command) + "," + argument);
        }
